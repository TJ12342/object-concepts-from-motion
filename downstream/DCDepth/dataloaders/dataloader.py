...
def preprocessing_transforms(mode, norm_image: bool = True):
    return transforms.Compose([
        ToTensor(mode=mode, norm_image=norm_image)
    ])
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            if self.args.dataset == 'kitti':
                ...
                if sample_path.split()[0].startswith('data_rgb'):
                    depth_file = os.path.join(
                        sample_path.split()[0].split('/')[0].replace('data_rgb', 'data_depth_annotated'),
                        sample_path.split()[0].split('/')[1], sample_path.split()[1])
                else:
                    depth_file = sample_path.split()[1]
                depth_file = os.path.join(sample_path.split()[0].split('/')[0], depth_file)
            ...
            if self.mode == 'online_eval':
                ...
                try:
                    ...
                    depth_gt = False
                    if not 'None' in depth_path:
                        print('Missing gt for {}, depth path: {}'.format(image_path, depth_path))
...
class ToTensor(object):
    def __init__(self, mode, norm_image):
        self.mode = mode
        self.normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]) \
            if norm_image else lambda x: x
