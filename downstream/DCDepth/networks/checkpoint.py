...
from mmcv.parallel import is_module_wrapper as is_model_wrapper
...
def load_checkpoint_swin(model,
    ...
    if sorted(list(state_dict.keys()))[0].startswith('model.backbone'):
        state_dict = {k.replace('model.backbone.', ''): v for k, v in state_dict.items() if k.startswith('model.backbone.')}
