...
def main():
    ...
    parser.add_argument('--resume_from', required=False, default=None)
    args = parser.parse_args()

    run_name_conf = os.path.splitext(os.path.basename(args.config))[0]
    # parse configs
    ...
    if cfgs.get('custom_imports', None):
        from mmcv.utils import import_modules_from_strings
        import_modules_from_strings(**cfgs['custom_imports'])
    ...
    if local_rank == 0:
        ...
        if args.resume_from is not None and os.path.isfile(args.resume_from):
            work_dir = os.path.dirname(args.resume_from)
        else:
            run_name = args.run_name
            # if not cfgs.debug and run_name == '':
            #     run_name = input('Name your run (leave blank for default): ')
            if run_name == '':
                run_name = run_name_conf + "-" + datetime.now().strftime("%Y-%m-%d/%H-%M-%S")
            else:
                run_name = run_name
            ...
            if os.path.exists(work_dir):  # must be an empty dir
                ...
                #     print('Bye.')
                #     exit(0)
                shutil.move(work_dir, work_dir + '_old_' + datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
    ...
    if cfgs.get('fp16', None):
        print("FP16 training is enabled...")
        optimizer_config = Fp16OptimizerHook(
            **cfgs.optimizer_config, **cfgs.fp16)
    else:
