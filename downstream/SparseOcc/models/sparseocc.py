...
class SparseOcc(MVXTwoStageDetector):
    def __init__(self,
        ...
        if img_neck_2 is not None:
            ...
            self.merge_conv = [torch.nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1).cuda() for _ in range(4)]
    ...
    def extract_img_feat(self, img):
        ...
        if self.img_neck_2 is not None:
            ...
            for i, img_feat in enumerate(img_feats):
                ...
                img_feat = F.interpolate(img_feats[i], size=(H_2, W_2), mode='bilinear', align_corners=False)
                img_feat = torch.cat((img_feat, img_feats_2[i]), dim=1)
                img_feat = self.merge_conv[i](img_feat)
                processed_feats.append(img_feat)
        else:
            processed_feats = img_feats
