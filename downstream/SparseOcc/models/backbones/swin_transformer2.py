...
from mmcls.models.utils import (ShiftWindowMSA, resize_pos_embed,
...
class SwinTransformer2(BaseBackbone):
    """Swin Transformer.
    ...
    Examples:
        >>> from mmpretrain.models import SwinTransformer2
        >>> import torch
        >>> extra_config = dict(
        >>>     arch='tiny',
        >>>     stage_cfgs=dict(downsample_cfg={'kernel_size': 3,
        >>>                                     'expansion_ratio': 3}))
        >>> self = SwinTransformer2(**extra_config)
    ...
    def __init__(self,
                 ...
                 patch_cfg=dict(),
                 init_cfg=None):
        super(SwinTransformer2, self).__init__(init_cfg=init_cfg)
    ...
    def init_weights(self):
        super(SwinTransformer2, self).init_weights()
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, *args,
        ...
        if (version is None
                or version < 2) and self.__class__ is SwinTransformer2:
            ...
            for k in state_dict_keys:
                if k.startswith('norm.') or k.startswith('backbone.norm.'):
                    ...
                    del state_dict[k]
        if (version is None
                or version < 3) and self.__class__ is SwinTransformer2:
            state_dict_keys = list(state_dict.keys())
            for k in state_dict_keys:
                if 'attn_mask' in k:
                    del state_dict[k]
