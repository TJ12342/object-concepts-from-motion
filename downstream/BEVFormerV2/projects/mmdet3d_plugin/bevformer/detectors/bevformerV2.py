...
class BEVFormerV2(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        self.img_backbone_2 = None
        self.img_neck_2 = None
        if img_backbone_2 is not None:
            self.img_backbone_2 = build_backbone(img_backbone_2)
        if img_neck_2 is not None:
            self.img_neck_2 = build_neck(img_neck_2)
            self.merge_conv = [torch.nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1).cuda() for _ in range(5)]
    ...
    def extract_img_feat(self, img):
        ...
        if img is not None:
            ...
            if self.img_backbone_2 is not None:
                img_feats_2 = self.img_backbone_2(img)
        ...
        if self.img_neck_2 is not None:
            img_feats_2 = self.img_neck_2(img_feats_2)

        img_feats_reshaped = []
        for i, img_feat in enumerate(img_feats):
            BN, C, H, W = img_feat.size()
            # img_feat_reshaped = img_feat.view(B, int(BN / B), C, H, W)
            if self.img_neck_2 is not None:
                BN_2, C_2, H_2, W_2 = img_feats_2[i].size()
                img_feat = F.interpolate(img_feat, size=(H_2, W_2), mode='bilinear', align_corners=False)
                img_feat = torch.cat((img_feat, img_feats_2[i]), dim=1)
                # print(img_feat.size())
                img_feat = self.merge_conv[i](img_feat)
                BN, C, H, W = img_feat.size()
                # print(img_feat.size())
            img_feat_reshaped = img_feat.view(B, int(BN / B), C, H, W)
    ...
    def forward_train(self,
        ...
        for x in range(len(img_feats)):
            losses['img_feats/norm' + str(x)] = torch.norm(img_feats[x], p=2).detach()
        return losses
